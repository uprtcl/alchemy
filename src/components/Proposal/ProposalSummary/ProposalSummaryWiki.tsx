...
import { EveesBindings, EveesRemote, EveesEthereum } from "@uprtcl/evees";
...
export default class ProposalSummaryWiki extends React.Component<IProps, IState> {
  ...
  async componentDidMount() {
    ...
    const eveesEthereum = uprtcl.orchestrator.container.getAll(
    ...
    this.setState({ uprtclProposalId:  decodedCallData.values[0], remoteId: eveesEthereum.id });
  }
  ...
  public render(): RenderOutput {
    ...
    switch (this.action.id) {
      case "authorizeProposal": {
        return (
          <module-container>
            <evees-proposal-diff remote-id={this.state.remoteId} proposal-id={this.state.uprtclProposalId}></evees-proposal-diff>
          </module-container>
        );
      }
      default:
